...
impl BilibiliParser {
    pub fn new(_proxy: &str) -> Result<Self> {
        Ok(Self { client: Client::builder().timeout(std::time::Duration::from_secs(30)).build()?, cookies: String::new() })
    }
    ...
    pub async fn parse(&self, url: &str, quality: &str) -> Result<VideoInfo> {
        ...
        let cid = info.cid;
        let play = self.fetch_play_url(&bvid, cid, quality).await?;
        Ok(VideoInfo { title: info.title, author: info.author, cover_url: info.cover_url, video_url: play.0, audio_url: play.1, selected_quality: play.2, platform: Platform::Bilibili, ..Default::default() })
    }

    async fn fetch_video_info(&self, bvid: &str) -> Result<BiliInfo> {
        let url = format!("https://api.bilibili.com/x/web-interface/view?bvid={}", bvid);
        let mut req = self.client.get(&url).header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/126 Safari/537.36").header("Referer", "https://www.bilibili.com/");
    ...
    async fn fetch_play_url(&self, bvid: &str, cid: i64, quality: &str) -> Result<(String, String, String)> {
        ...
        if let Some(dash) = data.get("dash") {
            let video_url = dash.get("video").and_then(|v| v.as_array()).and_then(|a| a.first()).and_then(|v| v.get("baseUrl").or(v.get("base_url"))).and_then(|v| v.as_str()).unwrap_or("").to_string();
            let audio_url = dash.get("audio").and_then(|v| v.as_array()).and_then(|a| a.first()).and_then(|v| v.get("baseUrl").or(v.get("base_url"))).and_then(|v| v.as_str()).unwrap_or("").to_string();
...
fn quality_to_qn(q: &str) -> i32 {
    match q { "4k" => 127, "1080p60" => 116, "1080p+" => 112, "1080p" => 80, "720p" => 64, "480p" => 32, "360p" => 16, _ => 80 }
}
fn qn_to_name(qn: i32) -> String {
    match qn { 127|120 => "4k", 116 => "1080p60", 112 => "1080p+", 80 => "1080p", 64 => "720p", 32 => "480p", 16 => "360p", _ => "auto" }.into()
}
