...
### 0. 发版必须经用户验收
- **禁止未经用户实测验收就公开发布**（GitHub Release、公开 tag、Docker 推送等）。
- 正确流程：本地构建 → 用户自己的服务器部署（可先测）→ **用户确认验收** → 才允许打 tag / 发 Release / 推送公开产物。
- 教训：YouTube 功能未经用户测试就发了 app-v1.6.0 和 v1.4.13，被要求回滚。
